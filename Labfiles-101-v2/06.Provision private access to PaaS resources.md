...
1. Right click on the **Forward Lookup Zones (2)** and select **New Zones (3)**.  
...
1.  From the **Administrator: Windows PowerShell ISE** console, run the following command to retrieve the DNS Server configured on the VM: 
...
1.  From the previous output, note the **Interface Index** for the **ServerAddress** and also grab the Lab-VM IP address recorded in previous step. 
