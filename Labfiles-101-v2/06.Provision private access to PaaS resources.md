...
1. Select **Virtual network Connections (1)** under **Connectivity** blade and click on **+ Add Connection (2)**
...
1. In the command prompt run the following command: 
   ...
   ```
    nslookup DB-VM-DeploymentID.onprem.local 
   ```  

   >**Note**: Make sure to replace **Deployment ID** with the Deployment ID present in **Environment details** page of the lab guide.Go to the **Environment details** Expand **Onprem-RG-<inject key="DeploymentID" enableCopy="false"/> (1)** and Copy **Deployment ID (2)**.

   ![Create resource](./Media/n58.png)
...
1. In the command prompt run the following command:   
   ...
   >**Note**: Make sure to replace **Deployment ID** with the Deployment ID present in **Environment details** page of the lab guide.Go to the **Environment details** Expand **RG-01-<inject key="DeploymentID" enableCopy="false"/> (1)** and Copy **Deployment ID (2)**.

   ![Create resource](./Media/165.png)
...
1. Go back to the **Powershell** and run the following command below to probe access to the storage account's private endpoint. 
   ...
   ```
   Test-NetConnection -ComputerName staccDeploymentID.blob.core.windows.net -Port 443 
   ```
   >**Note**: Make sure to replace **Deployment ID** with the Deployment ID present in **Environment details** page of the lab guide.
