...
1. Go back to **vwan-prod-001 | Virtual network Connections**, click on the drop-down symbol next to **Virtual networks (1)**, you can see **onprem-vnet-002 (2)**, **vnet-spoke3-<inject key="DeploymentID" enableCopy="false"/> (2)** and **vnet-spoke4-<inject key="DeploymentID" enableCopy="false"/> (2)** created.
...
1. In the **Windows PowerShell ISE** run the following command
   ...
   ```
   Test-NetConnection -ComputerName [Lab_VM_IP] -Port 3389  # For Windows-based VMs
   ```
